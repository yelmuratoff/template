...
- **Кандидат на отдельное решение (НЕ тайком)**: церемония фабрик (`AsyncFactory` + `InitializationHook` + `name` ради лога «🌀 Inited X») — самая тяжёлая часть кодбазы относительно пользы, наследие шаблона. Простые функции в `CompositionRoot` дали бы то же вдвое дешевле. Не трогается без явного решения (scope discipline).
