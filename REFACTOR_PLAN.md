...
- [ ] **Осталось пользователю**: ручной smoke `fvm flutter run --flavor dev -t lib/main_dev.dart` — splash → auth → login → табы → settings push → restart → restore → симуляция revoke → auth (рендер контейнерного узла без декларации — единственный непокрытый юнит-тестами рантайм-путь; делегат рендерит детей контейнера, как в эталонном примере)
